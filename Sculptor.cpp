...
r = g = b = a = 0;
  ...
  for(int i = 0; i < ny; i++){
    for(int j = 0; j < nx; j++){ 
      for(int k = 0; k < nz; k++){
        v[i][j][k].isOn = false; // inicializar cada elemento como vazio, ou falso
...
void Sculptor :: cutBox(int x0_, int x1_, int y0_, int y1_, int z0_, int z1_){
  ...
  void Sculptor :: writeOFF(const char* filename){
    ofstream fout;
    fixed(fout); // correção da precisão do .off
    int nver, nface, aux;
    nver = nface = aux = 0;
    fout.open(filename);
    fout<<"OFF"<<endl;
